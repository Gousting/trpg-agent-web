...
## 2026-08-05 桌面端 UI 大屏版

### 入口页（大屏配置层）
- index.html 内嵌全屏入口 overlay：直接访问显示配置表单（连接配置/游戏配置两栏），填完写回控制栏再开局；`?entry=1` 手机快捷路径不受影响
- 世界观切换实时显示剧本简介；主控角色选择实时预览角色卡（HP/SAN/技能）
- 暗黑羊皮纸 + 鎏金：噪点纹理 + 星云渐变 + 暗红雾团背景、38px 金线横幅标题、步骤条（连接配置→剧本与角色→开始跑团）、select/checkbox 自定义鎏金样式
- VLM 审查 6 轮：6.5 → 7.5（反 AI 味 4 条全过、舞台式布局认可）

### 游戏展示页（三栏化）
- 行动记录从底部移到右侧栏（与投票条同位，投票时覆盖），形成标准三栏：左角色/中场景/右日志
- 场景背景区右边界缩到日志面板左侧，消除日志压在场景上的视觉重叠
- 右日志面板改深色羊皮纸（与手机端一致），文本暖金配色
- 左角色面板 212→176px；.char-card.active 加金色外发光 + ▶ 指示
- scene-blur 桌面端禁用（仅移动端 contain 死黑边垫底）
- 修复 RemoteClient 空 key 发空 Bearer 头（Illegal header value）

### 验证
- VLM 桌面规则审查（4 反 AI 味 + 5 桌面硬规则 + 3 项目锚点，qwen3.7-plus + Go key）：5/10 → 6.5/10 → 9/10
- 300 passed；JS 语法通过；全远程 deepseek-v4-flash 跑局零错误
- 提交：c1aa8a5（入口页）、04a278c（游戏页）

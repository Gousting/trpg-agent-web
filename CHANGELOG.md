...
## v2.5 — 手机端 UI 改版：多角色胶囊状态栏 + 场景可见性 + 风格统一 (2026-08-07)

来源：用户反馈"外部展示不行"。VLM（qwen3.7-plus + 12 条 UI 规则包）三轮审查迭代。

### 新增

- **多角色胶囊状态栏**：手机端顶部动态渲染主控（金框高亮 HP+属性）+ AI 队友 HP 胶囊，`renderMobileStatus` 全量渲染 + `updateReinChip` 单刷，按 init 队伍名单过滤残留角色

### 修复

- 场景图被 blur(22px)/brightness(0.74) 模糊垫底盖成死灰 → 手机端关垫底 + cover 填满，桌面端提亮
- 记录框米色面板与暗黑场景割裂 → 深色羊皮卷 + 米金文字；高度 26→22dvh
- 进度条底部 4px 细条 → 顶部"守秘人编织中…"加载卡
- 入口页冷灰 → 暗黑羊皮纸 + 鎏金风格统一

### 验证

- 300 passed, 30 subtests；node --check 通过
- DOM 实测 3 胶囊（轮回者 12/12 + 林晓 10/10 + 王刚 15/15），陈明过滤
- VLM 规则审查通过（修复其提出的溢出截断/色调/标题层级）
